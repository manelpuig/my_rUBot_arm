...
class ArmPoseSequenceSimple(Node):

    def __init__(self):
        super().__init__("arm_pose_sequence")
        ...
        self.ik_link = self.common.get("ik_link", "tool")
    ...
    def _on_ik_result(self, future, step, name):
        ...
        if self.print_joints:
            ...
            for joint_name, value in zip(ARM_JOINTS, joint_goal):
                ...
                self.get_logger().info(f"  {joint_name}: {value_deg:.0f} deg ({value:.2f} rad)")
