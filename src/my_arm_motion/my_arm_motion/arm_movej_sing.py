...
class ArmMoveJSingularityChecked(ArmMoveJ):
    """MoveJ with sampled Jacobian, condition-number and joint-jump checks."""
    ...
    def _check_joint_jumps(self, positions):
        ...
        self.get_logger().info(
            f"Maximum planned joint jump: {maximum_jump_deg:.3f} deg "
            f"between points {maximum_segment} and {maximum_segment + 1}."
        )

        if maximum_jump_deg > self.max_joint_jump_deg:
            self.get_logger().error(
                f"Trajectory rejected: joint jump {maximum_jump_deg:.3f} deg "
                f"exceeds limit {self.max_joint_jump_deg:.3f} deg."
            )
            return False
    ...
    def _check_singularity_metrics(self, trajectory):
        ...
        self.get_logger().info(
            ...
            f"of {len(positions)} trajectory points..."
        )
        ...
        for index in indices:
            ...
            if (
                ...
                self.get_logger().error(
                    f"Trajectory rejected near point {index}: "
    ...
    def run(self):
        ...
        self.get_logger().info(
            f"MoveJ+singularity target in '{self.planning_frame}': "
        ...
        if not self._check_singularity_metrics(trajectory):
            self.get_logger().error(
                "MoveJ will not be executed because the trajectory failed "
        ...
        self.get_logger().info("Executing singularity-checked MoveJ trajectory...")
        ...
        self.get_logger().info(
            "Singularity-checked MoveJ trajectory execution succeeded."
        )
        return True
...
def main():
    rclpy.init()
    node = ArmMoveJSingularityChecked()
