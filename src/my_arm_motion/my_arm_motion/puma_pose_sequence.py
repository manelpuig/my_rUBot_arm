...
class PumaPoseSequenceSimple(Node):

    def __init__(self):
        super().__init__("puma_pose_sequence")
        ...
        self.group_name = self.common.get("group_name", "arm")
        self.ik_link = self.common.get("ik_link", "puma_tool")
        self.target_frame = self.common.get("target_frame", "base_link")
    ...
    def _get_seed_positions(self, step):
        ...
        if seed_from_joint_states and self._last_js is not None:
            ...
            if all(j in name_to_pos for j in PUMA_JOINTS):
                ...
                return [float(name_to_pos[j]) for j in PUMA_JOINTS]
    ...
    def _on_ik_result(self, future, step, name):
        ...
        if self.print_joints:
            ...
            for joint_name, value in zip(PUMA_JOINTS, joint_goal):
    ...
    def _send_trajectory(self, joint_goal, duration, step):
        ...
        goal.trajectory.joint_names = PUMA_JOINTS
        goal.trajectory.header.stamp = self.get_clock().now().to_msg()

        point = JointTrajectoryPoint()
        point.positions = joint_goal
        point.velocities = [0.0] * len(PUMA_JOINTS)
